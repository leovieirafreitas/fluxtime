...
import {
    ...
    LogOut, Menu,
    Plus, Search, Trash2, User, X, Zap, Coins, ExternalLink,
    Sun,
    Moon
} from 'lucide-react';
    ...
    useEffect(() => {
        const fetchDashboardData = async () => {
            ...
            try {
                ...
                if (cleanPhone) {
                    const { data: clientData } = await supabase.from('clients').select('created_at').eq('phone', parsed.phone).single();
        ...
        <div className={`min-h-screen font-sans ${theme === 'dark' ? 'bg-black text-white' : 'bg-slate-50 text-slate-900'}`}>
            ...
            <div className="max-w-7xl mx-auto p-4 md:p-8 flex gap-8 h-[calc(100vh-64px)] overflow-hidden">
                ...
                <aside className={`w-64 flex-shrink-0 flex-col gap-6 bg-slate-50 md:bg-transparent
                    ...
                    <div className={`px-4 py-3 border rounded-xl shadow-sm ${theme === 'dark' ? 'bg-neutral-900 border-neutral-800' : 'bg-white border-slate-200'}`}>
                ...
                <main className="flex-1 min-w-0 flex flex-col md:flex-row gap-6 overflow-hidden">
                    ...
                    <div className={`flex-1 rounded-xl border flex flex-col min-h-0 ${theme === 'dark' ? 'bg-black border-neutral-800' : 'bg-white border-slate-200'}`}>
                        ...
                        <div className={`p-4 border-b flex items-center justify-between ${theme === 'dark' ? 'border-neutral-800' : 'border-slate-100'}`}>
                            ...
                            <div className={`p-1 rounded-lg flex items-center gap-1 ${theme === 'dark' ? 'bg-neutral-900' : 'bg-slate-100'}`}>
                                {activeView === 'appointments' ? (
                                    <>
                                        ...
                                        <div className="relative">
                                            ...
                                            {isCalendarOpen && (
                                                <div className={`absolute top-full right-0 mt-2 p-4 rounded-xl shadow-xl border z-50 w-72 animate-scale-in ${theme === 'dark' ? 'bg-neutral-900 border-neutral-800' : 'bg-white border-slate-200'}`}>
                                                    {/* Calendar Header */}
                        ...
                        <div className="flex-1 overflow-y-auto px-6 space-y-0 relative pt-4">
                            ...
                            {activeView === 'appointments' ? (
                                <>
                                    {isLoading ? (
                                        <div className="flex items-center justify-center py-12 pl-[88px]">
                                            <div className="w-6 h-6 border-2 border-blue-600 border-t-transparent rounded-full animate-spin"></div>
                                        </div>
                                    ) : timelineEvents.length === 0 ? (
                                        <div className="py-12 pl-[88px] text-slate-500 text-sm">
                                                ...
                                                <div key={event.data.id} className="relative mb-2" ref={index === timelineEvents.length - 1 ? lastAppointmentRef : null}>
                                                    {showDateLabel && (
                                                        <div className="flex w-full items-center mb-6 mt-4">
                                                            <div className="w-[88px] pr-6 text-right text-xs text-slate-400 font-mono">
                                                    ...
                                                    <div className="flex w-full items-start group">
                                                        <div className="w-[88px] pr-6 text-right flex flex-col items-end pt-1">
                                                            <span className={`text-sm font-bold font-mono ${theme === 'dark' ? 'text-white' : 'text-slate-900'}`}>{formatTime(event.data.start_time)}</span>
                                                            <span className="text-[10px] text-slate-400">{formatTime(event.data.end_time)}</span>
                                                        </div>
                                                        ...
                                                        <div className="flex-1 pb-8">
                                                            <div className="flex items-center gap-2 mb-2 pl-4 text-sm w-full">
                                                                ...
                                                                <span className={`px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide rounded ${event.data.payment_status === 'paid'
                                                                ...
                                                                <div
                                                                    ...
                                                                    <div className="flex items-center justify-between">
                                                                        <div className="flex items-center gap-4">
                                                                            <div className={`flex flex-col items-center justify-center w-12 h-12 rounded-lg border ${theme === 'dark' ? 'bg-neutral-800 border-neutral-700 text-white' : 'bg-slate-50 border-slate-100 text-slate-700'}`}>
                                                                                <span className="text-[9px] uppercase font-bold text-blue-500">{getMonthAbbr(event.date)}</span>
                                                                                <span className="text-xl font-bold leading-none">{getDay(event.date)}</span>
                                                                            </div>
                                                                            <div>
                                                                                <div className="flex items-center gap-2">
                            ...
                            <div className="p-6 space-y-6">
                                ...
                                <div>
                                    <div className="flex items-center gap-2 mb-1">
                                        ...
                                        <span className={`px-2 py-0.5 text-[10px] font-bold rounded ${theme === 'dark' ? 'bg-red-900/30 text-red-400' : 'bg-red-50 text-red-600'}`}>Não pago</span>
                                    </div>
                                    <div className="flex items-center gap-3">
                                        ...
                                        <button className={`text-sm font-medium border px-3 py-0.5 rounded transition-colors ${theme === 'dark' ? 'text-blue-400 border-blue-800 hover:bg-neutral-800' : 'text-blue-600 border-blue-200 hover:bg-blue-50'}`}>
                                            Pagar
                                        </button>
                                    </div>
                                </div>
